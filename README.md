...
|---|---|---|
| **`'msprt'`** (the practical choice) | Mixture SPRT over the difference in means, prior δ ~ N(0, τ²), reject at Λ ≥ 1/α. Valid under repeated looks **given a known variance AND a Gaussian (or suitably sub-Gaussian) sampling model**. The mixture likelihood ratio is built from a Gaussian likelihood, so a known variance on its own is not enough, and Darwin has neither. | Darwin plugs in the observed Welch variance, and that is anti-conservative: when the within-arm spread comes out small by chance, Λ overshoots. **Measured under H0** (a coarse judge scoring {0, 0.1, 0.2} at {0.50, 0.05, 0.45}, default α=0.05 and warmup, checking after every individual run): type-I error **0.059 through n=14, 0.064 through n=20, 0.069 through n=30**. It is past α from the first horizon measured and keeps drifting. Through `SafetyGate` the primary test gets α/2, which brings the real error to about 0.046 rather than the 0.025 it was handed: the split narrows the gap, it does not buy calibration. **So this is not a calibrated test at Darwin's sample sizes**, it is a well-motivated stopping rule that behaves roughly like its nominal α over short horizons. v0.15 removed the worst of it (a branch that decided regardless of α); the rest needs a different method (unknown-variance e-process or t-mixture), not a patch. **v0.16 ships that method as `'eb'`, next row.** The numbers above are re-measured by the test suite on every run. |
| **`'eb'`** (v0.16: the method with both a proof and usable power) | Predictable plug-in empirical Bernstein confidence sequence ([Waudby-Smith & Ramdas, JRSS-B 2024, Theorem 2](https://arxiv.org/abs/2010.09686)). Time-uniform at level α by a nonnegative-supermartingale argument plus Ville's inequality, with **no i.i.d. assumption** (conditional means against the observation filtration) and **no variance plug-in inside the guarantee**: the bet λ_t adapts to the estimated spread, and because any predictable bet is valid, that estimate can only change the power, never the level. Bounds are enforced fail-closed exactly like `'hoeffding'`. | The supermartingale inequality itself is cited from the paper, not re-derived in four lines; [`tests/sequential-eb.test.ts`](tests/sequential-eb.test.ts) compensates by checking it numerically as an exact finite sum over a grid of laws, bets and predictable means, and by measuring the type-I error under continuous peeking (it stays at or below α at every horizon tried, unlike mSPRT's measured drift). Structural blind zone through **n = 17 per arm** on [0,1] at defaults, flagged via `inconclusiveByConstruction`. The width is **order-sensitive by design** (the bet is predictable from the prefix), so feed samples in chronological order; the gate already does. Measured decision points (same suite): constant arms 0.10 vs 0.95 at **n=21** (Hoeffding 32); σ≈0.05 with gap 0.30 at **n≈59** (Hoeffding 359); gap 0.20 at **n≈89** (Hoeffding 900); judge-noise σ≈0.10 with gap 0.10 at **n≈188** (Hoeffding 4216). The ~0.009 composite deltas our own fleet produces remain out of reach for every method here, EB included: the L/(c·n) floor does not shrink with the variance. What EB changes is that gaps in the 0.1 to 0.3 range become resolvable inside a real test's lifetime. |
